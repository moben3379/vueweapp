  ...
  <div id="main_tb">
    ...
    <div>
      <table>
        ...
        <tr v-for="(menu) in newdata" :key="menu.id">
          <td class="checkbox"><input  type="checkbox" :value="menu.id" v-model="checkedMenu"></td>
          <td>{{ menu.id }}</td>
          <td>{{ menu.parentId }}</td>
          <td>{{ menu.name }}</td>
          <td>{{ menu.url }}</td>
          ...
          <td>{{ menu.orderNum }}</td>
          <td>
            <button id="b_update" @click="update" style="background-color: rgb(0,136,255)"><i class=" fa fa-pencil"></i>
              修改
            </button>&nbsp;&nbsp;
            <button id="b_delete" @click="Delete" style="background-color: rgb(244,108,108)"><i class="fa fa-trash-o"></i>
              删除
            </button>
          </td>
        </tr>
    ...
    <div id="page">
      <ul class="pagination">
        <li>第{{ pageNum }}页</li>
        <li>
          <button @click="getpagedata(pageNum-1)" style="background-color: rgb(0,136,255)">上一页</button>
        </li>
        <li v-for="m in addpage()" :key="m"><a href="#" @click="getpagedata(m)">[{{ m }}]</a></li>
        <li>
          <button @click="getpagedata(pageNum+1)" style="background-color: rgb(0,136,255)">下一页</button>
        </li>
      </ul>
    </div>
